...
import 'package:elbi_donation_app/donor/builders/qr_code_generator.dart';
...
class _DonorDonationFormState extends State<DonorDonationForm> {
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              //           MaterialPageRoute(builder: (_) => CameraPage(callback:(imageFile) {setState(() => information["photo"] = imageFile as String);}, cameras: value))
              //         )
              //       );
              //     },
              //     child: const Text("Take a Picture", style: TextStyle(fontSize: 15, color: Colors.white, fontWeight: FontWeight.bold)),
              //   )
              // ),
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        if(_formKey.currentState!.validate()) {
                          ...
                          showDialog(
                            context: context, 
                            builder: (BuildContext context) => createAlertDialogForm(context, donationID)
                          );
                        }
                      }, 
                      style: ButtonStyle(
  ...
  Widget createAlertDialogForm(BuildContext context, String donationID) {
  ...
  return AlertDialog(
        ...
        child: Column(
          ...
          children: [
            ...
            if (information["donationmethod"] == "Drop-off") generateQRcode(donationID),
              ...
              //   onQRCodeSaved: (qrImgUrl) {
              //     if (qrImgUrl != null) {
              //       context.read<DonationsProvider>().editQRimg(donationID, qrImgUrl);
              //     }
              //   },
              // ),
          ],
    ...
    actions: <Widget>[
      TextButton(
        onPressed: () {
           _saveQRImage(donationID).toString();
  ...
  Future<String?> _saveQRImage(String donationID) async {
    try {
      ...
      if (_repaintBoundaryKey.currentContext != null) {
        ...
        String? downloadURL = await context.read<DonationsProvider>().addQRimg(imageData, donationID);
